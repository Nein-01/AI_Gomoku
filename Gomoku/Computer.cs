...
namespace Gomoku
{
    class Computer
    {
        ...
        public int Minimax_AB(int r, int c, bool isMax, int alpha, int beta, int depth = 1)
        {
            if (game.CheckWin(r, c))
            {
                return (isMax ? -1 : 1) * 1000000000;
            }
            ...
            if (isMax)
            {
                ...
                for (int i = 0; i < moveList.Count; i++)
                {
                    ...
                    maxValue = Math.Max(maxValue, Minimax_AB(row, col, false, alpha, beta, depth + 1));
            ...
            else
            {
                ...
                for (int i = 0; i < moveList.Count; i++)
                {
                    ...
                    int col = moveList[i].column;
                    game.NextMove(row, col);
                    minValue = Math.Min(minValue, Minimax_AB(row, col, true, alpha, beta, depth + 1));
                    game.RemoveMove(row, col);
        ...
        public Move NextMove()
        {
            ...
            for (int i = 0; i < moveList.Count; i++)
            {
                ...
                int col = moveList[i].column;
                game.NextMove(row, col);
                int value = Minimax_AB(row, col, false, alpha, beta) + random.Next(-1, 2);
                game.RemoveMove(row, col);
                if (value > max)
                {
                    max = value;
